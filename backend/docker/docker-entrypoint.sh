...
show_help() {
# If you change this please update ./docs/reference/digit-docker-api.md
    echo """
...
if [ "$MIGRATE_ON_STARTUP" = "true" ] ; then
  echo "python /digit/backend/src/digit/manage.py migrate"
  python /digit/backend/src/digit/manage.py migrate
fi
# if [ "$SYNC_TEMPLATES_ON_STARTUP" = "true" ] ; then
#   echo "python /digit/backend/src/digit/manage.py sync_templates"
#   python /digit/backend/src/digit/manage.py sync_templates
# fi
}
...
case "$1" in
    dev)
        ...
        CMD="python /digit/backend/src/digit/manage.py runserver 0.0.0.0:${PORT}"
        echo "$CMD"
    ...
    local)
        wait_for_postgres
        run_setup_commands_if_configured
        exec gunicorn --workers=3 -b 0.0.0.0:"${PORT}" -k uvicorn.workers.UvicornWorker digit.config.asgi:application
    ;;
    bash)
        exec /bin/bash "${@:2}"
    ;;
    manage)
        exec python /digit/backend/src/digit/manage.py "${@:2}"
    ;;
    python)
        exec python "${@:2}"
    ;;
    shell)
        exec python /digit/backend/src/digit/manage.py shell
    ;;
    lint)
